// ============================================
// PDF BUILDER — Geschäft Teil 1: Im Geschäft (A2/B1)
...
(function () {
    ...
    function buildTheory() {
        ...
        return `<h1 class="chapter">📘 1. Teorie — Im Geschäft (Grundwortschatz)</h1>` + t;
    }
    ...
    function buildDialog() {
        ...
        dialog1Data.replici.forEach(r => {
            const sp = r.speaker === 'andreea' ? 'Andreea 🇷🇴' : 'Frau Wagner 🛒';
    ...
    function buildExercises() {
        ...
        // EX1 — Lückentext (before/after/answer)
        ...
        // EX2 — Multiple Choice (q/options/correct/explanation)
        ...
        html += `<div class="ex-block"><h3>Übung 3 — Potrivește vocabularul</h3><div class="rezolvare-banner">✓ Rezolvare</div><table><thead><tr><th style="width:45%">🇩🇪 Wort</th><th>🇷🇴 Traducere</th></tr></thead><tbody>`;
        if (typeof ex3Pairs !== 'undefined') ex3Pairs.forEach(p => { html += `<tr><td class="verb">${p.de}</td><td class="ro-text">${p.ro}</td></tr>`; });
        html += `</tbody></table></div>`;

        // EX4 — Traduceri RO→DE (q/answer)
        ...
        // EX5 — Situație ↔ expresie
        html += `<div class="ex-block"><h3>Übung 5 — Situație ↔ expresie</h3><div class="rezolvare-banner">✓ Rezolvare</div><table><thead><tr><th style="width:50%">🇩🇪 Expresie</th><th>🛒 Situație</th></tr></thead><tbody>`;
    ...
    function buildFlashcards() {
        let html = `<h1 class="chapter new-section">📇 4. Vocabular complet (Flashcards)</h1>
            <p style="margin-bottom:14px">Cele <strong>${typeof flashcardsData !== 'undefined' ? flashcardsData.length : 0} de carduri</strong> ale lecției — vocabularul de bază din magazin.</p>
    ...
    function buildVerbs() {
        let html = `<h1 class="chapter new-section">🔁 5. Verb-Konjugation — verbe din magazin</h1>
            <div class="theory-box warn-box"><p><strong>📌 Reamintire:</strong> helfen e TARE și cere DATIV (Ihnen). Prefixele neseparabile (ver-, be-) → Perfekt FĂRĂ „ge-". Verbele în -ieren (anprobieren) → tot fără „ge-". Separabile (umtauschen) → „-ge-" la mijloc. Präteritum = IMPERFECT.</p></div>`;
